...
[<Rpc>] 
let TestTweet (tweet) = async { printfn "TeseTweetPrint: %s" tweet //return { Name = tweet }
                              }

[<Rpc>] 
let plus () = userID <- userID + 1

[<Rpc>] 
let changefake () = async { fake <- 0  }

[<Rpc>] 
let rpcToString () =    userID <- userID + 1
                        userID.ToString()                              
...
module Client =
    ...
    let Main (aboutPageLink: string) wsep =
        IndexTemplate.Body()
            ...
            .Login(Login)
            .Register(fun _ ->   WebSocketClient.WebSocketTest (username.Value+","+Login.Value+",Register", wsep)|> ignore)
                                //Remote<RpcUserSession>.Login Login.Value |> Async.Start)
            .Tweet(fun t ->   WebSocketClient.WebSocketTest (username.Value+","+Login.Value+",Tweet", wsep) |> ignore
                                //TestTweet(Login.Value) |>ignore          
                    )
            .ReTweet(fun t ->   WebSocketClient.WebSocketTest (username.Value+","+Login.Value+",ReTweet", wsep) |> ignore     
                    )
            .Subscribe(fun t ->   WebSocketClient.WebSocketTest (username.Value+","+Login.Value+",Subscribe", wsep) |> ignore     
                    )
            .Query(fun t ->   WebSocketClient.WebSocketTest (username.Value+", ,Query", wsep) |> ignore     
                    )
            .QueryTag(fun t ->   WebSocketClient.WebSocketTest (username.Value+","+Login.Value+",QueryTag", wsep) |> ignore     
                    )
            .QueryAt(fun t ->   WebSocketClient.WebSocketTest (username.Value+","+Login.Value+",QueryAt", wsep) |> ignore     
                    )
            .GetLogin(fun t ->   WebSocketClient.WebSocketTest (username.Value+","+Login.Value+",Login", wsep) |> ignore     
                    )
            .Logout(fun t ->   WebSocketClient.WebSocketTest (username.Value+","+Login.Value+",Logout", wsep) |> ignore     
                    )
            // .GetLogin(fun _ ->
            //     async {
            ...
            .WebSocketTest( //plus ()
                            let name = rpcToString()
                            Var.Set username name
                            WebSocketClient.WebSocketTest (username.Value, wsep))
